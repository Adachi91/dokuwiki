...
ReadtheDokus.prototype.getMediaQuery = function() {
    const mediaQueryElement = document.querySelector("#__media_query");

    if (!mediaQueryElement) {
        console.warn("Media query element not found!");
        return "pc"; // Or a sensible default like "pc"
    }

    return getComputedStyle(mediaQueryElement)
        .getPropertyValue("--media-query")
        .trim();
};
// -----------------------------------------------------------------------------
